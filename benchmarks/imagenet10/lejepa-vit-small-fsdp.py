"""LeJEPA ViT-Small on ImageNet-10 (Imagenette) under FSDP.

Identical to ``benchmarks/imagenet10/lejepa-vit-small.py`` except for the
training strategy: this variant runs the encoder under FSDP via
:func:`make_fsdp_strategy`. Compare line-by-line with the DDP/auto variant
to see exactly what the FSDP path requires.

Scales transparently to multi-GPU when more devices become available:

- ``num_gpus = 1`` (default): FSDP wraps the ViT but with ``world_size=1``
  there is no sharding so communication collectives are no-ops.
- ``num_gpus >= 2``: FSDP shards parameters / gradients / optimizer state
  across ranks. Per-block wrapping is auto-detected from the timm ViT
...
from stable_pretraining.utils.fsdp import default_auto_wrap_policy, make_fsdp_strategy
...
def lejepa_forward(self, batch, stage):
    ...
    Identical to ``benchmarks/imagenet10/lejepa-vit-small.py``'s
    ``lejepa_forward`` — duplicated here so this script is independently
    runnable.
    """
    out = {}
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=str(
                    Path(__file__).parent / "checkpoints" / "lejepa-vits-fsdp"
                ),
