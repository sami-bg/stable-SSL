...
imagenet_train = spt.data.HFDataset(
    path="clane9/imagenet-100",
    split="train",
    transform=train_transform,
)

imagenet_val = spt.data.HFDataset(
    path="clane9/imagenet-100",
    split="validation",
...
train_dataset = IndexedDataset(imagenet_train, transform=train_transform)
val_dataset = IndexedDataset(imagenet_val, transform=val_transform)
...
module = spt.Module(
    ...
    optim={
        ...
        "scheduler": {
            ...
            "peak_step": (warmup_epochs * len(train)) // num_devices,
            "total_steps": (num_epochs * len(train)) // num_devices,
        },
        "interval": "step",
    },
)
