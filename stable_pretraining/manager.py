...
class Manager(submitit.helpers.Checkpointable):
    ...
    def _matches_template(ckpt_name: str, callback: ModelCheckpoint) -> bool:
        ...
        if ckpt_stem.startswith(f"{callback.CHECKPOINT_NAME_LAST}-v") and callback.save_last:
            return True

        # Case 3: templated filename case
        # Get the template from the callback, using the default if not set.
        template = callback.filename or "{epoch}"+callback.CHECKPOINT_JOIN_CHAR+"{step}"
        ...
        if is_mc_explicitly_configured and ckpt_path:
            for callback in trainer.callbacks:
                if isinstance(callback, ModelCheckpoint):
                    ...
                    if ckpt_path.parent == resolved_dirpath and self._matches_template(ckpt_path.name, callback):
