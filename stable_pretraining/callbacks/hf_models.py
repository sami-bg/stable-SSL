...
class HuggingFaceCheckpointCallback(Callback):
    """
    A PyTorch Lightning callback that identifies submodules inheriting from 
    Hugging Face's `PreTrainedModel` and exports them into standalone, 
    "zero-knowledge" loadable HF directories.
    ...
    Example:
        ...
        ...     "checkpoints/hf_models/step_5000/backbone", 
    ...
    def __init__(self, save_dir: str = "hf_exports", verbose: bool = True):
        ...
        logger.info(f"Initialized HF Checkpoint Callback. Target: <cyan>{self.save_dir}</cyan>")

    def _get_hf_submodules(self, pl_module: pl.LightningModule) -> Dict[str, PreTrainedModel]:
        """Identifies top-level children that are instances of PreTrainedModel."""
        return {
            name: module for name, module in pl_module.named_children() 
    ...
    def _log_discovery_table(self, submodules: Dict[str, PreTrainedModel]):
        ...
        if not submodules:
            logger.warning("No Hugging Face (PreTrainedModel) submodules found in LightningModule.")
            return
        ...
        for name, mod in submodules.items():
            print(f"| {name:<18} | {mod.__class__.__name__:<22} | {mod.config.__class__.__name__:<22} |")
        print(sep)
    ...
    def on_save_checkpoint(self, trainer: pl.Trainer, pl_module: pl.LightningModule, checkpoint: Dict[str, Any]):
        """
        Triggered by Lightning's checkpointing logic. Creates an atomic 
        HF-compatible export for every found submodule.
        """
        ...
        for name, model in hf_submodules.items():
            ...
            logger.success(f"Step {step}: Exported HF submodule '<green>{name}</green>' -> {model_save_path}")